...
where $f_1$ is $L$-smooth, $\mu$-strongly convex and quadratic,
...
```
"""
function wc_proximal_gradient_quadratics(L, mu, gamma, n; solver=Clarabel.Optimizer, verbose=true)


    problem = PEP()


    f1 = declare_function!(problem, SmoothStronglyConvexQuadraticFunction, OrderedDict("mu" => mu, "L" => L))
